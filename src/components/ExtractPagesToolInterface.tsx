...
import { extractPages, getPdfInfo } from '@/app/actions/pdf-actions';
import { Label } from '@/components/ui/label';

export const ExtractPagesToolInterface = () => {
  ...
  const handleProcess = async () => {
    ...
    try {
      const indicesToExtract = Array.from(selectedPages);
      const result = await extractPages(file.base64, indicesToExtract);
      setProgress(100);
      ...
      if (result.success && result.dataUrl) {
        ...
        addDocument({
          ...
          thumbnail: thumbnails[Array.from(selectedPages)[0]] || file.base64,
          content: `Pages extracted via CyberAegXs Engine.`,
          size: "Optimized",
          type: 'pdf'
        });

        toast({ title: "Success", description: `${selectedPages.size} pages extracted securely.` });
      } else {
  ...
  const downloadResult = () => {
    ...
    link.href = resultUrl;
    link.download = `CyberAegXs_Extracted_${file?.name || 'document'}.pdf`;
    link.click();
  };
      ...
      <div className="w-full max-w-6xl">
        ...
        <div className="bg-card border border-border rounded-3xl p-8 md:p-12 relative overflow-hidden shadow-2xl">
          <header className="mb-10 flex flex-col items-center text-center">
            ...
            <h2 className="text-4xl font-black tracking-tighter mb-4 uppercase">Extract PDF Pages</h2>
            <p className="text-zinc-500 max-w-lg">Select exactly which pages you want to pull into a new document. Our engine ensures high-fidelity cloning of selected assets.</p>
          </header>
          ...
          {!isProcessing && !isComplete && !file && (
            <div className="border-2 border-dashed border-zinc-800 rounded-3xl p-12 flex flex-col items-center text-center cursor-pointer hover:border-primary/60 hover:bg-zinc-900/50 transition-all bg-zinc-950/50 group">
              ...
              <h3 className="font-bold text-xl mb-1">Select PDF to extract from</h3>
            ...
            <div className="space-y-10">
              <div className="flex items-center justify-between p-4 bg-zinc-900/50 rounded-2xl border border-white/5">
                ...
                <div className="flex items-center gap-4">
                  <div className="hidden md:flex flex-col items-end">
                    <span className="text-[10px] font-bold text-primary uppercase">{selectedPages.size} PAGES SELECTED</span>
                    <span className="text-[10px] font-bold text-zinc-600 uppercase">FROM {pageCount} TOTAL</span>
                  </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between px-2">
                  <Label className="text-xs font-bold uppercase tracking-widest text-zinc-500 flex items-center gap-2">
                    <Eye className="h-3 w-3" /> Select pages to extract
                  </Label>
                      ...
                      >
                        <div className={cn(
                          ...
                          isSelected 
                            ...
                            : "border-zinc-800 group-hover:border-primary/50 group-hover:scale-[1.01]"
                        )}>
                          {thumbnails[pageNum] ? (
                            <img src={thumbnails[pageNum]} alt={`Page ${pageNum}`} className={cn(
                              "w-full h-full object-cover transition-all duration-300",
                              isSelected ? "opacity-100" : "opacity-60 group-hover:opacity-100 grayscale-[0.5] group-hover:grayscale-0"
                            )} />
                          ) : (
                          ...
                          <div className={cn(
                            ...
                            isSelected ? "opacity-100 bg-primary/10" : "opacity-0 group-hover:opacity-100 bg-primary/5"
                          )}>
                            {isSelected ? (
                              <div className="flex flex-col items-center gap-2">
                                <CheckCircle2 className="h-10 w-10 text-primary animate-in zoom-in-75" />
                                <span className="text-[8px] font-black text-primary tracking-tighter uppercase bg-black/60 px-2 py-0.5 rounded-full">Selected</span>
                              </div>
                            ) : (
              ...
              {selectedPages.size === 0 ? (
                <div className="flex items-center justify-center gap-3 p-6 rounded-2xl border border-dashed border-zinc-800 bg-zinc-900/20">
                  ...
                  <p className="text-sm font-medium text-zinc-500">Select at least one page to begin extraction.</p>
                </div>
              ) : (
                <Button 
          ...
          {isProcessing && (
            <div className="py-24 flex flex-1 flex-col items-center justify-center gap-8">
              ...
              <div className="w-full max-w-sm space-y-4 text-center">
                <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest animate-pulse">Isolating Selection Bytes...</p>
          ...
          {isComplete && (
            <div className="py-12 flex flex-col items-center text-center space-y-8 animate-in zoom-in-95 duration-500">
              ...
              <div>
                <h3 className="text-3xl font-black uppercase tracking-tighter">CLONE COMPLETE</h3>
                <p className="text-zinc-500 mt-2 font-medium">Selected pages extracted and verified for structural integrity.</p>
              </div>
              <div className="flex flex-col sm:flex-row gap-4 w-full">
                <Button onClick={downloadResult} className="flex-1 bg-accent text-accent-foreground font-black h-16 text-sm uppercase tracking-widest hover:scale-[1.02] hover:bg-accent/90 transition-all">
                  <Download className="mr-3 h-5 w-5" /> DOWNLOAD EXTRACTED PDF
                </Button>
...
// Hallucinated icon fix (Plus is in lucide, check for consistency)
